...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"BtcfCoinAddress\"\n"
            "\nReturn information about the given BtcfCoin Address.\n"
            "\nArguments:\n"
            "1. \"BtcfCoinAddress\"     (string, required) The Btcf address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"BtcfCoinAddress\", (string) The BtcfCoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BtcfCoin Address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are BtcfCoin Addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) BtcfCoin Address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"BtcfCoinAddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"BtcfCoinAddress\"  (string, required) The BtcfCoin Address to use for the signature.\n"
