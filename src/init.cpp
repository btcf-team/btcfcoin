...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "btfccoin.conf") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-wuwuwu") || mapArgs.count("-whitewuwuwu")) {
    ...
    int nBind = std::max((int)mapArgs.count("-wuwuwu") + (int)mapArgs.count("-whitewuwuwu"), 1);
    ...
    if (fListen) {
        if (mapArgs.count("-wuwuwu") || mapArgs.count("-whitewuwuwu")) {
            BOOST_FOREACH(std::string strBind, mapMultiArgs["-wuwuwu"]) {
                CService addrBind;
                if (!Lookup(strBind.c_str(), addrBind, GetListenPort(), false))
                    return InitError(strprintf(_("Cannot resolve -bind address: '%s'"), strBind));
                fBound |= Bind(addrBind, (BF_EXPLICIT | BF_REPORT_ERROR));
            }
            BOOST_FOREACH(std::string strBind, mapMultiArgs["-whitewuwuwu"]) {
                CService addrBind;
